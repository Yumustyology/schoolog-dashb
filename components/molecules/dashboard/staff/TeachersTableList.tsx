...
function TeachersTableLists() {
  ...
  const teachersList: TeacherRow[] = React.useMemo(() => {
  ...
  const notYetAvailable = (label: string) => () =>
    showToast(`${label} isn't available yet`, `staff-${label.toLowerCase()}-soon`, {
      type: 'info',
    });
  ...
  const changeStatus = (id: string, status: string, successMessage: string) => async () => {
    try {
      ...
      showToast(successMessage, `staff-status-${id}`, { type: 'success' });
      mutate();
    } catch {
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        ...
        const menuItems = [
          {
            label: 'View Profile',
            onClick: () => router.push(`/school/teaching-staffs/${row._id}`),
