...
const YNmodal: React.FC<ModalProps> = ({ isOpen, onClose, children, title, body }) => {
        ...
        <div className="fixed inset-0 bg-[rgb(0,0,0,0.25)] flex items-center  justify-center z-50 w-full ">
            <div className="bg-white rounded-lg tablet:w-[434px]   xxs:w-full shadow-lg">
                ...
                <main className='flex flex-col items-center justify-center text-center px-6 '>
                    ...
                    <h4 className={cn('text-[16px] text-black mb-3 ', Inter_500.className)}> {title}</h4>
                    <p className={cn('text-sm text-[#434343] mx-8', Inter_400.className)}>{body}</p>

                </main>
                <div className='w-full mt-8 mb-6 text-center flex justify-center gap-4 '>
                    <Button round className={cn('bg-transparent border text-primary text-xl border-primary h-[44px] w-[185px]', Inter_500.className)}> Cancel </Button>
                    <Button round className={cn('text-xl bg-primary text-white h-[44px] w-[185px]', Inter_500.className)}> Submit </Button>
                </div>
            </div>
        </div>
    );
