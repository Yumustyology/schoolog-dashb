      ...
      {tableDescription.map((description) => (
        <div key={description.id}>
          <div className="flex justify-between items-center mb-6 border-[#E0E0E0] border p-3.5 rounded-md">
            <div>
              <h4 className={cn('text-sm text-gray6 mb-1.5', Inter_500.className)}>
                {description.class} {description.term} term result
              </h4>
              <div className={cn('flex items-center gap-2 text-gray3 text-xs', poppins_400.className)}>
                <span className="text-gray3 text-xs">
                  {description.number_of_subjects} subjects
                </span>
                <Dot size={1} />
                <span className="text-gray3 text-xs">{description.date} - {description.time}</span>
              </div>
            </div>
            <div className="flex items-center gap-3.5">
              {openTable[description.id] &&
                <Button round className={cn('text-primary text-[16px] flex gap-4 bg-primary1', Inter_600.className)}>
                  <ScreenIcon /> <span>Full screen</span>
                </Button>
              }
              <Button round className={cn('text-white text-[16px] flex gap-4 bg-primary', Inter_600.className)}>
                <DownloadIcon size="20" color="#ffffff" /> <span>Download</span>
              </Button>
              <div className="cursor-pointer" onClick={() => toggleDrawer(description.id)}>
