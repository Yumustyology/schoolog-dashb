...
import { updateAcademicYear } from '@/app/lib/actions/academicYear.actions';
...
import { normalizeAcademicYear } from '@/app/lib/utils/academic-years.utils';
import {
  deleteTermSession,
  updateTermSession,
} from '@/app/lib/actions/term-session.actions';
...
const EditAcademicYearForm: React.FC<EditAcademicYearFormProps> = ({
  initialData,
  onSuccess,
  countryCode,
}) => {
  ...
  const normalized = useMemo(
    () => normalizeAcademicYear(initialData),
    [initialData]
  );
  console.log('Normalized Academic Year:', normalized);
  // Memoize initial values to prevent recreation on every render
  const initialValues = useMemo<AcademicYearFormValues>(
    () => ({
      ...
      terms:
        normalized?.terms && normalized.terms.length
          ? normalized.terms
          : [
              {
  ...
  // Track original term IDs to detect deletions
  const originalTermIds = useRef(
    new Set((normalized?.terms || []).map((t) => t._id).filter(Boolean))
  );

  const formik = useFormik({
    enableReinitialize: false, // ✅ Changed from true to false
    initialValues,
    validate: (values: any) => {
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        await updateAcademicYear(normalized?._id!, academicYearPayload);

        const currentTermIds = new Set(
          values.terms.map((t) => t._id).filter(Boolean)
        );
        const deletedTermIds = Array.from(originalTermIds.current).filter(
          (id) => !currentTermIds.has(id)
        );

        const deletePromises = deletedTermIds.map((termId) =>
          deleteTermSession(termId as string)
        );
        await Promise.all(deletePromises);

        const termPromises = values.terms.map(async (term) => {
          const termPayload = {
            name: term.name,
          ...
          if (term._id) {
            return updateTermSession(term._id, termPayload);
          }
  ...
  return (
    <form className="space-y-5" onSubmit={formik.handleSubmit}>
      ...
      <div>
        ...
        >
          <AdditionIcon color="black" size={14} /> Add another term
        </button>
      </div>
