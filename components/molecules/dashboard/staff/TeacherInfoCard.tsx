...
export const TeacherInfoCard = ({ staff, onStatusChanged }: TeacherInfoCardProps) => {
  const name = `${staff.firstName} ${staff.lastName}`.trim();
  const assignedClass = staff.classes?.find((c) => c.isClassTeacher) || staff.classes?.[0];
  const assignedClassName =
    assignedClass && typeof assignedClass.classGradeId === 'object'
      ? assignedClass.classGradeId.name
      : '-';
  ...
  return (
    <div className="bg-white py-6 h-[390px] pb-10 px-6 rounded-md col-span-2 border-none">
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className={cn('text-sm text-gray', poppins_400.className)}>
              {staff.isTeachingStaff ? 'Teacher' : 'Non-teaching staff'}
            </p>
          </div>
        </div>
      ...
      <SingleInfo
        ...
        rightText="Assigned class"
        rightValue={assignedClassName || 'None'}
      />
      <SingleInfo
        leftText="Date joined"
        leftValue={staff.createdAt ? formatDate(staff.createdAt) : '-'}
      />
