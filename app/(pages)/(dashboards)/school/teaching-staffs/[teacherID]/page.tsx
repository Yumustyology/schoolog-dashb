...
const TeacherInfoPage = () => {
  ...
  const { data, isLoading, mutate } = useSWR(
    teacherID ? `/staff/${teacherID}` : null,
    () => staffActions.fetchStaffById(teacherID)
  );
          ...
          crumbs={[
            {
              label: 'Staff',
              isActive: false,
              href: '/school/teaching-staffs',
            },
            {
              label: staff?.staffSlugId || teacherID,
              isActive: true,
            },
          ]}
        />
        ...
        <div>
          <div className="flex space-x-3 mt-4">
            <div className="w-[446px]">
              <TeacherInfoCard staff={staff} onStatusChanged={() => mutate()} />
            </div>
            <div className="flex-1 ">
              <TeacherOthersInfoCard staff={staff} />
            </div>
          </div>
